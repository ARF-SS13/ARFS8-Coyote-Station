...
/datum/preference/toggle/be_round_removed/is_accessible(datum/prefs_holder/preferences)
	if (!..())
		return FALSE

	return (CONFIG_GET(flag/use_rr_opt_in_preferences))

/datum/preference/toggle/be_round_removed/deserialize(input, datum/prefs_holder/preferences)
	if (!CONFIG_GET(flag/use_rr_opt_in_preferences))
		return CONFIG_GET(flag/rr_opt_level_default)

	return ..()

/datum/preference/toggle/be_round_removed/apply_to_human(mob/living/carbon/human/target, value, datum/prefs_holder/preferences)
	return FALSE
