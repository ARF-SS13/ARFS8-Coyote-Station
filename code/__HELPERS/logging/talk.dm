 ...
 *
 * This proc reads the `message_data` to determine
 ...
 */
/atom/proc/log_sayverb_talk(message, list/message_data = list(), tag = null, log_globally = TRUE, forced_by = null)
	// If it's just the custom say verb, log it to emotes.
	if(message_data[MODE_CUSTOM_SAY_ERASE_INPUT])
		log_talk(message_data[MODE_CUSTOM_SAY_EMOTE], LOG_RADIO_EMOTE, tag, log_globally, forced_by)
		return

	if(message_data[WHISPER_MODE])
		log_talk(message, LOG_WHISPER, tag, log_globally, forced_by, message_data[MODE_CUSTOM_SAY_EMOTE])
	else
		log_talk(message, LOG_SAY, tag, log_globally, forced_by, message_data[MODE_CUSTOM_SAY_EMOTE])
