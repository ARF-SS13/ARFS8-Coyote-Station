...
/obj/item/assembly/voice/Hear(atom/movable/speaker, message_language, raw_message, radio_freq, radio_freq_name, radio_freq_color, list/spans, list/message_data = list(), message_range)
	. = ..()
	if(message_data[WHISPER_MODE] || message_data[MODE_RELAY]) //Too quiet lad
		return FALSE
	if(speaker == src)
		return FALSE
