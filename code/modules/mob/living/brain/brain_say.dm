...
/mob/living/brain/radio(message, list/message_data = list(), list/spans, language)
	if(message_data[MODE_HEADSET] && istype(container, /obj/item/mmi))
		var/obj/item/mmi/R = container
		if(R.radio)
			R.radio.talk_into(src, message, language = language, message_data = message_data)
